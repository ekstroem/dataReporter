...
#' prefixed with "reporteR_", if this qualifies as a valid file name (e.g. no special
...
#'  supplied and \code{vol=2}, the output file will be called "reporteR_myData2.Rmd"
#'
  ...
  if (!is.null(treatXasY)) {
    if (!is.list(treatXasY)) {
      ...
      warning(paste("The treatXasY argument specified for: ",
                    ...
                    "are not supported by reporteR and therefore ",
...
##########################################################################################
  ...
  if (is.null(file)) {
    if (substr(dfname, 1, 11) == "data.frame(") {
      file <- paste0("reporteR_unnamedData", vol, ".Rmd")
    } else file <- normalizeFileName(paste0("reporteR_", dfname, vol, ".Rmd"))
  } else {
    originalFile <- file
    faultyExt <- FALSE
    faultyName <- FALSE
...
#  }
  ...
  if (replace) {
    ...
    stop(paste("The file name(s) to be used by reporteR,", paste(problemFiles, ",", sep=""),
               "is(are) already in use.",
               "We recommend trying one of the following solutions: \n",
               "- rename your reporteR output file using the \"file\" option \n",
  ...
  try({
    ...
    if (standAlone) {
      ...
      writer("subtitle: \"Autogenerated data summary from reporteR\"")
...
#        writer("  - \\usepackage{fullpage}")
    ...
    if ("visualize" %in% mode & "standardVisual" %in% visuals) {
      secretChunk.wrapper(c("ggAggHist <- getFromNamespace(\"ggAggHist\", \"reporteR\")", 
...
##              chunk_name <- paste0("Var-", idx)
    ...
    desc <- lapply("reporteR", packageDescription, lib.loc = NULL, encoding = NA)
    ...
    writer(" *  reporteR v", version, " [Pkg: ", pkgdate, " from ", pkgsource, "]\n")
  ...
  if (output %in% c("html", "pdf") && render) {
    ##is it possible to close the file reporteR_data.pdf/html if it is open such
