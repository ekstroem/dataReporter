...
reporteR_as_factor <- function(x, levels = c("default", "labels", "values", "both"),
                               ordered = FALSE, ...) {
  ...
  if (levels == "default" || levels == "both") {
    ...
    vals <- unique(x)
    levs <- reporteR_haven_replace_with(vals, unname(labels), names(labels))
    # Ensure all labels are preserved
    levs <- sort(c(stats::setNames(vals, levs), labels), na.last = TRUE)
    levs <- unique(names(levs))
    
    x <- reporteR_haven_replace_with(x, unname(labels), names(labels))
    ...
    x <- reporteR_haven_replace_with(x, levs, labs)
...
## Adding a verbatim copy of the unexported function reporteR_haven_replace_with
## so a note does not pop up when checking the package.
reporteR_haven_replace_with <- function(x, from, to) 
{
