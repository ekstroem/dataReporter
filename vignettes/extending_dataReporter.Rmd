...
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Extending dataReporter}
...
## Introduction
The data documentation tools available in `dataReporter` allows for it to be utilized as an easy-to-use, self-contained package, as outlined in our accompanying article in the _Journal of Statistical Software_, **90**(6), [doi:10.18637/jss.v090.i06](https://www.jstatsoft.org/article/view/v090i06). However, this does *not* imply that `dataReporter` is not customizable: `dataReporter` was specifically built for including user-made extensions and this vignette is a tutorial into how one can write simple extensions that can be used in e.g. the `makeDataReport()` function from `dataReporter`. 

### Three steps of data documentation
In order to understand how different parts of the report creation can be customized, we will briefly dwell on the structure of the data reports made by `dataReporter`. Aside from a few pages dedicated to gaining an overview of the full dataset and what was done to document its current state, the report sequentially presents information for each variable. More specifically, each variable is presented with one of the two sets of information:

1. The result of a single eligibility check (referred to as a *pre-check* below)
2. The results of the three canonical `dataReporter` steps:
...
In this vignette, we focus on the report creation tools of `dataReporter`. However, it should be emphasized that by following the guidelines presented here, the user-defined `summaryFunctions`, `visualFunctions` and `checkFunctions` will also be fully integrated with the interactive mode of `dataReporter`. 
...
be part of the full machinery of `dataReporter`, and when this is done, the functions will
be recognized and behave in the same way as the built-in functions in
`dataReporter`. The presentation below is given in the format of
...
As mentioned above, `dataReporter` provides a dedicated class for
...
all supported classes in `dataReporter`:
...
`dataReporter` output document. Writing a `visualFunction` is slightly
...
The last, but perhaps most important, `dataReporter` function type is
...
myProbValCheckFunction <- function(v, nMax, maxDecimals, ...) {
  ...
  problemMessage <- "[Message that is printed prior to listing
                     problem values in the dataReporter output,
                     ending with a colon]"
...
the new function such that it gets the full `dataReporter`
...
of all the seven variable classes supported by `dataReporter`, and we use `setdiff()` to choose all
...
We will make a data report for the built-in dataset `artData` that is based on the *Master Works of Art* data from [Data Explorer](https://www.data-explorer.com/data/). The dataset contains 200 observations (corresponding to paintings) of 11 variables, describing different properties of the paintings and their artists. The first 5 observations look like this:
...
```{r makeReport, message  = FALSE, warning = FALSE, eval=rmarkdown::pandoc_available()}
...
We include the report documenting `artData` (this is only shown if `pandoc` is available to generate it):

```{r includeReport, eval=rmarkdown::pandoc_available()}
