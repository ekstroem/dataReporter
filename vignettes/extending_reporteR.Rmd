...
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Extending reporteR}
...
In this vignette, we focus on the report creation tools of `reporteR`. However, it should be emphasized that by following the guidelines presented here, the user-defined `summaryFunctions`, `visualFunctions` and `checkFunctions` will also be fully integrated with the interactive mode of `reporteR`. 
...
be part of the full machinery of `reporteR`, and when this is done, the functions will
be recognized and behave in the same way as the built-in functions in
`reporteR`. The presentation below is given in the format of
...
As mentioned above, `reporteR` provides a dedicated class for
...
all supported classes in `reporteR`:
...
`reporteR` output document. Writing a `visualFunction` is slightly
...
The last, but perhaps most important, `reporteR` function type is
...
myProbValCheckFunction <- function(v, nMax, maxDecimals, ...) {
  ...
  problemMessage <- "[Message that is printed prior to listing
                     problem values in the reporteR output,
                     ending with a colon]"
...
the new function such that it gets the full `reporteR`
...
of all the seven variable classes supported by `reporteR`, and we use `setdiff()` to choose all
...
## A reporteR report with user-defined functions: Documenting artData
...
htmltools::includeHTML("reporteR_artData.html")
```
